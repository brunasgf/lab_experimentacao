...
''''''
# save gitHub repository urls in .txt file
...
def export_to_csv(data):
    ...
    for node in data:
        ...
        git_path = node['url'] + ".git"  
        clone_repository(git_path, repo_path)

        call_ck_metrics(repo_path)
        metrics_df = pd.read_csv(os.path.abspath(os.getcwd()) + "/class.csv", usecols=['cbo', 'dit', 'lcom', 'loc'])
